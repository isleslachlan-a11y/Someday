  ...
  return (
    <>
      <button
        ...
        className="flex items-center justify-center w-11 h-11 rounded-full hover:bg-white/[0.06] text-muted hover:text-white-soft transition-colors"
      >
        <PenSquare size={20} />
      </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center">
          ...
          {/* Sheet */}
          <div className="relative w-full max-h-[70vh] lg:w-[420px] lg:max-h-[560px] bg-[#130f2a] rounded-t-3xl lg:rounded-2xl border-t border-white/[0.07] lg:border flex flex-col shadow-2xl">
            {/* Drag handle (mobile) */}
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07] shrink-0">
              <h2 className="font-syne text-lg font-bold text-white-soft">New message</h2>
              <button
                onClick={() => setIsOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto">
              {isPending && friends === null ? (
                ...
                <div className="px-5 py-10 text-center">
                  ...
                  <p className="text-muted/60 text-xs mt-1">Add friends from the Profile page</p>
                </div>
              ) : (
                <div className="divide-y divide-white/[0.05]">
                  {friends.map(friend => (
                    <button
                      ...
                      disabled={isPending}
                      className="w-full flex items-center gap-3 px-5 py-4 hover:bg-white/[0.03] text-left transition-colors disabled:opacity-60"
                    >
                      <Avatar
