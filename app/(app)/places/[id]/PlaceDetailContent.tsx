...
export default function PlaceDetailContent({
  ...
  // activities prop kept for future use
  void activities
  ...
  async function handleSaveSimilar(placeId: string) {
    if (savedSimilarIds.has(placeId)) {
      ...
      void logEvent(userId, 'item_removed', { place_id: placeId, source: 'detail_similar' })
          ...
          <div className="pt-6">
            <div className="flex items-center justify-between px-4 mb-3">
              ...
              <span className="font-nunito text-[#131936]/40 text-[12px]">
                {childExperiencePlaces.length}{' '}
                {childExperiencePlaces.length === 1 ? 'experience' : 'experiences'}
              </span>
            </div>
            {childExperiencePlaces.length > 0 ? (
              ...
              <div className="mx-4 rounded-2xl bg-[#fcd99a]/20 border border-[#fcd99a]/40 p-5 text-center">
                <p className="font-nunito text-[#131936]/40 text-[13px]">
                  No experiences added yet for {place.name}
                </p>
              </div>
            )}
          </div>
