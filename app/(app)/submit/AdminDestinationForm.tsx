...
function ActivityAdder({ placeId }: { placeId: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <button type="button" onClick={() => void addActivity()} disabled={saving || !name.trim()} className="w-full h-11 rounded-full bg-[#f08c21] text-[#131936] font-syne font-bold text-[14px] disabled:opacity-50">
          ...
          <div className="space-y-3">
            ...
            {manualImageValid && manualImageUrl && (
              <div className="rounded-2xl overflow-hidden border border-[#fcd99a]">
                <div className="relative h-40"><Image src={manualImageUrl} alt="Manual URL preview" fill className="object-cover" onError={() => { toast.error('Could not load image from that URL.'); setManualImageValid(false) }} /></div>
              </div>
            )}
          </div>
        )}
