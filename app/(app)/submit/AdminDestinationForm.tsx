      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          <label htmlFor="admin-dest-name" className="font-nunito text-[12px] text-[#131936]/50 mb-1 block">
            Place name <span className="text-[#f89a14]">*</span>
          </label>
          <div className="relative">
            <input
        ...
        <div>
          <label className="font-nunito text-[12px] text-[#131936]/50 mb-2 block">Country <span className="text-[#f89a14]">*</span></label>
          {locationLocked ? (
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={() => setShowAllTags(prev => !prev)} className="font-nunito text-[11px] text-[#f89a14] hover:opacity-80 transition-opacity">
          ...
          {selectedTags.length > 0 && (
            <p className="font-nunito text-[11px] text-[#131936]/50 mt-2">
              ...
              <button type="button" onClick={() => setSelectedTags([])} className="text-[#f89a14]">clear all</button>
            </p>
          )}
        </div>
        {allLabels.length > 0 && (
        ...
        {imageTab === 'unsplash' && (
          <>
            <div className="flex gap-2 mb-3">
              ...
              <button type="button" onClick={() => void searchImages()} disabled={imageSearchLoading} className="px-4 py-2.5 rounded-full bg-[#f89a14] text-[#131936] font-nunito font-semibold text-[13px] shrink-0 disabled:opacity-50">
            ...
            {visibleImages.length > 0 && (
              <>
                ...
                {hasMore && <button type="button" onClick={() => setImagePage(p => p + 1)} className="w-full py-2 font-nunito text-[13px] text-[#f89a14] hover:opacity-80 transition-opacity">Show more images ({allImageResults.length - visibleImages.length} more)</button>}
              </>
            )}
            {selectedImage && (
              <div className="rounded-2xl overflow-hidden border border-[#fcd99a] mb-2 mt-3">
                ...
                <p className="font-nunito text-[#131936]/40 text-[10px] px-3 py-1.5">
                  Photo by <a href={selectedImage.attribution.photographer_url} target="_blank" rel="noopener noreferrer" className="text-[#f89a14]">{selectedImage.attribution.photographer_name}</a> on Unsplash
                </p>
              </div>
            )}
          </>
          ...
          <div className="space-y-3">
            ...
            {uploadFile && (
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={uploadConsent} onChange={e => setUploadConsent(e.target.checked)} className="mt-0.5 w-4 h-4 accent-[#f89a14] shrink-0 cursor-pointer" />
