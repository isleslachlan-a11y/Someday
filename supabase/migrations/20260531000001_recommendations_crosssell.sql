-- Cross-sell / pull-through signal for recommendations.
-- Experiences linked (via parent_place_id) to destinations the user has saved
-- get a 0.15 boost. Weight breakdown (must sum to 1.0):
--   category match  0.30
--   tag match       0.18
--   social graph    0.17
--   trending        0.08
...
DROP FUNCTION IF EXISTS get_recommendations_for_user(uuid, integer);
...
BEGIN
  ...
  WITH
    ...
    -- Cross-sell: experiences whose parent destination is already saved
    crosssell AS (
      SELECT
        p.id AS experience_id,
        0.15 AS crosssell_score
      FROM public.places p
      JOIN public.bucket_list_items bli ON bli.place_id = p.parent_place_id
      WHERE bli.user_id  = p_user_id
        AND p.type        = 'experience'
        AND p.parent_place_id IS NOT NULL
        AND p.id NOT IN (SELECT experience_id FROM saved)
    ),
    cat_scores AS (
    ...
    scored AS (
      SELECT
        p.id AS experience_id,
        COALESCE(cs.cat_score,      0) * 0.30
          + COALESCE(ts.tag_score,  0) * 0.18
          + COALESCE(fs.social_score, 0) * 0.17
          + CASE WHEN p.trending THEN 0.08 ELSE 0 END
          + COALESCE(m.momentum_score, 0) * 0.12
          + COALESCE(cx.crosssell_score, 0) * 0.15
          AS score,
        CASE
          WHEN cx.crosssell_score IS NOT NULL THEN 'crosssell'
      ...
      LEFT JOIN crosssell    cx ON cx.experience_id = p.id
